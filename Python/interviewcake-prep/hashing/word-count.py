...
class WordCloudData(object):
    ...
    def __init__(self, input_string):
        self.strings = list(input_string)
    ...
    def remove_punctuation(self):
        for i in range(len(self.strings)):
            if not self.strings[i].isalpha():
                if i+1 < len(self.strings):
                    if self.strings[i - 1].isalpha() and self.strings[i + 1].isalpha():
                        pass
                    else:
                        self.strings[i] = ' '
                else:
        ...
        self.strings = ''.join(self.strings).split()
        for i in self.strings:
            if i[0].isupper():
                if i.lower() in self.strings:
                    print(i)
                    self.strings.remove(i)
                    self.strings.append(i.lower())
        # print(self.strings)
    ...
    def word_count(self):
        ...
        for i in self.strings:
            ...
            # print(i)
            if self.words_to_counts.get(i) is not None:
                self.words_to_counts[i] += 1
            elif i not in '.!?\\-':
                self.words_to_counts.setdefault(i, 1)
